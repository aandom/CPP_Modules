...
void    ClapTrap::beRepaired(unsigned int amount) {
    ...
    if (this->_hit + amount > ClapTrap::defaultHitP) {
        amount = ClapTrap::defaultHitP - this->_hit;
    }
    if (amount == 0) {
        this->_energy--;
        std::cout << this->_name << " tries to repair while the amount to repair is 0 or it doen't need any repair" << std::endl;
        return ;
    }
    this->_energy--;
    this->_hit += amount;
