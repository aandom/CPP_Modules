...
/*   Updated: 2023/10/19 21:30:42 by aandom           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    Span::addNumber(std::list<int>::const_iterator itbegin, std::list<int>::const_iterator itend) {
    ...
    if (std::abs(std::distance(itbegin, itend)) > (int) (this->_arrsize - this->_array.size())) {
        std::string  errmsg = YELLOW "Error: Insuficient space to hold " + std::to_string(std::abs(std::distance(itbegin, itend))) + " more elements" RESET;
...
unsigned int Span::shortestSpan() const {
	...
	if (this->_array.size() < 2)
		throw (std::range_error( YELLOW "Cannot compute Shortest Span: small list size" RESET));
...
unsigned int Span::longestSpan() const {
	...
	if (this->_array.size() < 2)
		throw (std::range_error(YELLOW "Cannot compute longest Span: small list size" RESET));
